...
Item {
    ...
    function applyWallpaper(safeFileName, isVideo) {
        ...
        if (window.currentFilter === "Search" && window.hasSearched) {
            ...
            let mapFile = paths.getCacheDir("wallpaper") + "/search_map.txt";
            ...
            if (alreadyExists) {
                const applyScript = `
                    ...
                    cp "$DEST_FILE" ${paths.getCacheDir("wallpaper")}/current_wallpaper.png || true
                    pkill mpvpaper || true
        ...
        const thumbFile = paths.getCacheDir("wallpaper") + "/thumbs/" + safeFileName;
        ...
        const fullScript = `
            cp "${isVideo ? escThumb : escOriginal}" ${paths.getCacheDir("wallpaper")}/current_wallpaper.png || true
            pkill mpvpaper || true
    ...
    onIsSearchPausedChanged: {
        Quickshell.execDetached(["bash", "-c", "echo '" + (isSearchPaused ? "pause" : "run") + "' > " + paths.getRunDir("wallpaper") + "/ddg_search_control"]);
    }
    ...
    function triggerOnlineSearch() {
        ...
        const cmd = `
            ...
            echo 'stop' > ${paths.getRunDir("wallpaper")}/ddg_search_control
            ...
            echo 'run' > ${paths.getRunDir("wallpaper")}/ddg_search_control
    ...
    readonly property string thumbDir: "file://" + paths.getCacheDir("wallpaper") + "/thumbs"
    readonly property string searchDir: "file://" + paths.getCacheDir("wallpaper") + "/search_thumbs"
    ...
    FolderListModel {
        id: markerModel
        folder: "file://" + paths.getCacheDir("wallpaper") + "/colors_markers"
        showDirs: false
        nameFilters: ["*_HEX_*"]
    ...
    function triggerColorExtraction() {
        const extractScript = `
            COLOR_DIR="${paths.getCacheDir('wallpaper')}/colors_markers"
            THUMBS="${paths.getCacheDir('wallpaper')}/thumbs"
            CSV="${paths.getCacheDir('wallpaper')}/colors.csv"
    ...
    Component.onDestruction: {
        if (window.hasSearched) {
            ...
            Quickshell.execDetached(["bash", "-c", "echo 'pause' > " + paths.getRunDir("wallpaper") + "/ddg_search_control"]);
        } else {
            Quickshell.execDetached(["bash", "-c", "echo 'stop' > " + paths.getRunDir("wallpaper") + "/ddg_search_control; for p in $(pgrep -f ddg_search.sh); do if [ \"$p\" != \"$$\" ] && [ \"$p\" != \"$BASHPID\" ]; then kill -9 $p 2>/dev/null || true; fi; done; pkill -f '[g]et_ddg_links.py'"]);
        }
    }
}
