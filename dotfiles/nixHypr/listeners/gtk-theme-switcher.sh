...
apply_theme() {
    ...
    if [[ "$THEME_PREF" == "1" || "$THEME_PREF" == "true" ]]; then
        ...
        if [ -f $HOME/.cargo/bin/matugen ]; then
            $HOME/.cargo/bin/matugen image $(cat ~/.cache/nixHypr/hyprland-dotfiles/current_wallpaper) --source-color-index 0 -m "dark"
        else
            matugen image $(cat ~/.cache/nixHypr/hyprland-dotfiles/current_wallpaper) --source-color-index 0 -m "dark"
        fi
        ...
        # Update NixHypr Dotfiles Settings theme
        qs -p $HOME/.local/share/nixHypr-dotfiles-settings/quickshell ipc call theme-manager reload
        echo "NixHypr Dotfiles Settings Theme updated"

        # Reload Bar
        nohup bash -c "$HOME/.config/nixHypr/scripts/launch-bar" > /dev/null 2>&1 &
        disown
        ...
        if [ -f $HOME/.cargo/bin/matugen ]; then
            $HOME/.cargo/bin/matugen image $(cat ~/.cache/nixHypr/hyprland-dotfiles/current_wallpaper) --source-color-index 0 -m "light"
        else
            matugen image $(cat ~/.cache/nixHypr/hyprland-dotfiles/current_wallpaper) --source-color-index 0 -m "light"
        fi
