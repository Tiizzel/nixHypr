...
Item {
    ...
    function applyPresetOptimistically(presetName) {
        ...
        if (presets[presetName]) {
            ...
            execCmd(`$HOME/.config/quickshell/nixHypr-shell/music/equalizer.sh preset ${presetName}`);
        }
    }
    ...
    Process {
        id: musicProc
        running: true
        command: ["bash", "-c", "$HOME/.config/quickshell/nixHypr-shell/music/music_info.sh"]
    ...
    Process {
        id: eqProc
        running: true
        command: ["bash", "-c", "$HOME/.config/quickshell/nixHypr-shell/music/equalizer.sh get"]
        stdout: StdioCollector {
            onStreamFinished: {
                if (this.text) {
                            ...
                            Slider {
                                ...
                                onPressedChanged: {
                                    if (pressed) {
                                        ...
                                        root.execCmd(`$HOME/.config/quickshell/nixHypr-shell/music/player_control.sh seek ${value.toFixed(2)} ${root.musicData.length} "${safePlayer}"`);
                            ...
                            MouseArea {
                                ...
                                onClicked: {
                                    if (root.eqData.pending) {
                                        ...
                                        root.triggerEqLightning();
                                        root.execCmd("$HOME/.config/quickshell/nixHypr-shell/music/equalizer.sh apply");
                                    }
                                }
                            }
                        }
                                        ...
                                        Slider {
                                            ...
                                            onPressedChanged: {
                                                if (!pressed) {
                                                    ...
                                                    root.execCmd(`$HOME/.config/quickshell/nixHypr-shell/music/equalizer.sh set_band ${modelData.idx} ${Math.round(value)}`);
                                                }
                                            }
