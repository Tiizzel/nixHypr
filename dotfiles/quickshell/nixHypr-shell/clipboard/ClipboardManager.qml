...
Item {
    ...
    Process {
        id: clipFetcher
        running: true
        command: ["python3", Quickshell.env("HOME") + "/.config/quickshell/nixHypr-shell/clipboard/clip_fetcher.py", window.currentOffset, window.fetchLimit, paths.getCacheDir("clipboard")]
    ...
    function loadMore() {
        ...
        clipFetcher.command = ["python3", Quickshell.env("HOME") + "/.config/quickshell/nixHypr-shell/clipboard/clip_fetcher.py", window.currentOffset, window.fetchLimit, paths.getCacheDir("clipboard")];
    ...
    Connections {
        ...
        function onVisibleChanged() {
            if (window.visible) {
                ...
                clipFetcher.command = ["python3", Quickshell.env("HOME") + "/.config/quickshell/nixHypr-shell/clipboard/clip_fetcher.py", 0, window.fetchLimit, paths.getCacheDir("clipboard")];
                clipFetcher.running = true;
            } else {
                searchInput.text = "";
                window.pendingIndex = -1;
